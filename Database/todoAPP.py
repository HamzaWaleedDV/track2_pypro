import sqlite3
from pathlib import Path

message = '''
"a" => Add New Task
"d" => Delete A Task
"s" => Show All Tasks
"u" => Update A Task
"q" => Quit The App
please choose option: 
'''

user_input = input(message).strip().lower()

#Create commands
commands_list = ["a", "d", "s", "u", "q"]
user_id = 2
try:
    sqliteConnection = sqlite3.connect(Path.home() / Path('OneDrive', 'Desktop', 'TodoAPP.db'))
    crsr = sqliteConnection.cursor()
except:
    print("Connection error")
finally:
    if (sqliteConnection):
        #create table
        sql_command = """CREATE TABLE if not exists tasks(
        user_id INTEGER,
        task_name VARCHAR(20),
        description TEXT
        )"""
        crsr.execute(sql_command)


        def show_tasks():
            crsr.execute(f"SELECT * FROM tasks WHERE user_id='{user_id}'")

            results = crsr.fetchall()

            print(f"You have {len(results)} tasks")

            if len(results) > 0:
                for task in results:
                    print(f'Task Name: {task[1]}, AND', end=' ')
                    print(f'Task Description: {task[2]}')

            sqliteConnection.commit()

        def add_task():
            task_name = input("Write task name: ").strip()
            des = input("Write Description For Task: ").strip()

            crsr.execute(F"INSERT INTO tasks (user_id, task_name, description) VALUES ('{user_id}', '{task_name}', '{des}')")

            sqliteConnection.commit()


        def delete_task():
            task_name = input('Write The Name Of Task You Need To Delete: ')

            crsr.execute(f"DELETE FROM tasks WHERE task_name='{task_name}' and user_id='{user_id}'")
            sqliteConnection.commit()

        def update_task():
            task_name = input("Write The Task Name You Want To Modify: ").strip()

            crsr.execute(f"SELECT * FROM tasks WHERE task_name = '{task_name}' and user_id = '{user_id}'")
            results = crsr.fetchall()
            if not results:
                print("There is no task with this name")
            else:
                des = input("Write a new description: ").strip()
                crsr.execute(f"UPDATE tasks SET description = '{des}' WHERE task_name = '{task_name}' AND user_id = '{user_id}' ")
                sqliteConnection.commit()
                print("The task has been successfully modified")


        def end_app():
            print("Program closed")
            exit()

        if user_input in commands_list:
            if user_input == "a":
                add_task()
            elif user_input == "s":
                show_tasks()
            elif user_input == "d":
                delete_task()
            elif user_input == "u":
                update_task()
            elif user_input == "e":
                end_app()

        else:
            print("Sorry This Command Is Not Found") 
    
    sqliteConnection.close()